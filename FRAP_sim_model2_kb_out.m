...
function output = FRAP_sim_model2_kb_out(k, p, n, Pclust)
...
for t = 2 : 1: n
    R_free(t)  = R_free(t-1) + k2*0R_elong(t-1) + k4*R_clust(t-1) - (k3 + k1)*R_free(t-1);
...
output.free_total  = R_free/N;
output.elong_total = R_elong/N;
output.clust_total = R_clust/N;

output.Pfree = mean(R_free(end-40:end))/N;
output.Pelong = mean(R_elong(end-40:end))/N;
output.Pclust = mean(R_clust(end-40:end))/N;
